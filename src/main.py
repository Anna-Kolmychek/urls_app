...
async def create_url(url_item: schemas.UrlItemCreate, db: AsyncSession = Depends(get_db)):
    ...
    db_url_item = await utils.get_url_item_by_origin(db, origin_url=url_item.origin_url)
    if db_url_item:
        raise HTTPException(status_code=400, detail="Url already registered")
    url_item = await utils.create_url_item(db=db, url_item=url_item)
    return url_item
...
async def read_url(url_item_id: int, db: AsyncSession = Depends(get_db)):
    ...
    db_url_item = await utils.get_url_item(db, url_item_id=url_item_id)
